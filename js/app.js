...
function _safeExtNav(){
  try{
    // ⚠️ _hasUnsavedChanges는 let 선언이라 window 객체에 안 붙음 → bare 이름으로 직접 할당해야 함
    _hasUnsavedChanges = false;
...
// 변경 이력이 없으면 emp.join 그대로 반환 (fallback)
function _earliestJoinFor(emp){
  if(!emp) return null;
  const candidates = [];
  if(typeof EMP_CHANGE_HISTORY !== 'undefined' && Array.isArray(EMP_CHANGE_HISTORY)){
    for(const h of EMP_CHANGE_HISTORY){
      if(String(h.empId)===String(emp.id) && h.field==='join' && h.before && h.before!=='-'){
        candidates.push(h.before);
      }
    }
  }
  if(emp.join) candidates.push(emp.join);
  if(candidates.length === 0) return null;
  // YYYY-MM-DD 문자열은 사전순 = 시간순
  candidates.sort();
  return candidates[0];
}
// ───────────────────────────────────────────────────────────────────────────
...
function monthSummary(eid,y,m){
  ...
  // 입사일 이전 월이면 빈 결과
  // 가장 이른 입사일 기준 (변경이력 + 현재 emp.join 중 제일 오래된 것) — 2026-05-20-07
  {const _ej=_earliestJoinFor(emp);if(_ej){const jd=parseEmpDate(_ej);if(jd>new Date(y,m,0))return{wdays:0,adays:0,aldays:0,twkH:0,tNightH:0,tOtDayH:0,tOtNightH:0,tHolDayH:0,tHolNightH:0,tHolDayOtH:0,tHolNightOtH:0,tBase:0,tNightPay:0,tOtDayPay:0,tOtNightPay:0,tHolDayPay:0,tHolNightPay:0,tHolDayOtPay:0,tHolNightOtPay:0,annualPay:0,wkly:0,bonus:0,allowances:{},totalAllowance:0,deduction:0,total:0};}}
  // 퇴사일 이후 월이면 빈 결과
...
function renderTable(){
  ...
  const activeDayEmps = applyCommonFilter(EMPS.filter(emp=>{
    // 가장 이른 입사일 기준 (2026-05-20-07)
    const _ej=_earliestJoinFor(emp);
    if(_ej){const jd=parseEmpDate(_ej);if(jd>dayDate)return false;}
...
function renderPayroll(){
  ...
  const activePayEmps = applyCommonFilter(EMPS.filter(emp=>{
    // 가장 이른 입사일 기준 (2026-05-20-07)
    const _ej=_earliestJoinFor(emp);
    if(_ej){const jd=parseEmpDate(_ej);if(jd>payMonthEnd)return false;}
...
function renderXlPreview(){
  ...
  const payEmps = applyCommonFilter(EMPS.filter(emp=>{
    // 가장 이른 입사일 기준 (2026-05-20-07)
    const _ej=_earliestJoinFor(emp);
    if(_ej){const jd=parseEmpDate(_ej);if(jd>new Date(pY,pM,0))return false;}
...
function exportDailyExcel(){
  ...
  const activeDayEmps = applyCommonFilter(EMPS.filter(emp=>{
    // 가장 이른 입사일 기준 (2026-05-20-07)
    const _ej=_earliestJoinFor(emp);
    if(_ej){const jd=parseEmpDate(_ej);if(jd>dayDate2)return false;}
...
function openShiftModal(type){
  ...
  if(type==='edit'&&shiftSelected.size>0){
    ...
    if(emp){
      ...
      document.querySelectorAll('.sm-day-btn').forEach(btn=>{
