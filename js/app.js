...
function renderXlPreview(){
  ...
  const rows = payEmps.map((emp,idx)=>{
    ...
    return `<tr>
      ...
      <td class="num">${(Math.round(basePay/(emp.sot||POL.sot||209)/10)*10).toLocaleString('ko-KR')}</td>
...
function importLeaveFromExcel(){
  ...
  if(unmatched.size) report+=`미매칭 이름: ${[...unmatched].join(', ')}\n`;
  report+='\n=== 직원별 사용연차 검증 ===\n';
  [...matched].sort().forEach(name=>{
    const emp=EMPS.find(e=>(e.name||'').trim()===name);
    if(!emp)return;
    const used=countUsedLeave(emp.id,year);
    const lv=calcLeaveForYear(emp,year);
    report+=`${name}: 사용 ${used}일 / 총 ${lv.total}일 / 잔여 ${lv.remain}일\n`;
  });
  alert(report);
  console.log(report);
