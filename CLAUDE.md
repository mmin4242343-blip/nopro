...
| 태그 | 의미 | 멤버 (이메일) |
|------|------|--------------|
| `mm_group` | 명민 그룹 — wehago 연동 운영 | `test2@naver.com`, `mmings@naver.com`, `nadabsy@naver.com`, `kij0825@naver.com` (4개, 최대 9개로 확장 예정) |
| `beta_split_load` | PDF 방안 2 — 초기 로드 분할 베타 (2026-05-19 도입) | `testnam1@naver.com` (1주일 운영 관찰 후 확대 결정. 상세는 아래 "PDF 방안 2 진행 상황" 섹션) |
...
## PDF 방안 2 — 초기 로드 분할 진행 상황 (2026-05-19 진행 중)

작성 기준: `docs/backend-stability-plan.pdf` 방안 2 (응답시간 7~9초 → 0.5~1초, 504 위험 99% 감소)

### 현재 단계
**Day 5 (통합 테스트 + 회귀 점검) — `testnam1@naver.com`에서 1주일 운영 관찰 중**
- 시작: 2026-05-19
- 관찰 종료 예정: ~2026-05-26
- 베타 활성 대상: `testnam1@naver.com` (단일)

### 완료된 작업 (Day 1~4 + 핫픽스 3건)
| 단계 | 내용 | 커밋 |
|------|------|------|
| Day 1 | `ESSENTIAL_KEYS`(6개)·`REMAINDER_KEYS`(14개) 분류, `sbLoadEssential`·`sbLoadRemainder` 함수 신설. `sbLoadAll`은 호환성 래퍼로 유지 | `a244b35` |
| Day 2 | `doAuthLogin`/`initAuth`에 `groupTag === 'beta_split_load'` 분기. 베타만 분할 로드 | `fe0fea2` |
| Day 3 | `_needsRemainder`·`_isBetaSplitLoad`·`_showRemainderPlaceholder` + `nopro:remainder-loaded` 이벤트 핸들러 | `f078c60` |
| Day 4 | 페이지별 메시지(`_REMAINDER_MESSAGES`) + 베타 한정 시간 측정 콘솔 로깅 | `6f4bbe1` |
| fix 1 | daily 자동 재렌더 (gp() 'daily' 분기 누락 우회 — 이벤트 핸들러에서 `renderTable()` 직접 호출) | `effef70` |
| fix 2 | **빈값 저장 근원 차단** — `sbSaveAll`/`safeItemSave` 진입에서 `_syncedSnapshot===null` 즉시 return. `_filter` 가드는 백업으로 유지 (다중 방어). 모든 회사 적용 | `82e9d8d` |
| fix 3 | `showAuthModal`/`showLanding` SES 환경 호환 — `window.X = X` 명시 노출 | `1889218` |

### 추후 작업 (1주일 관찰 후 결정 단계)

**확대 결정 절차:**
1. **1단계** — 큰 회사 1~2곳 추가 (직원 100명+ 또는 rec 데이터 큰 회사). PDF가 약속한 5~10배 속도 효과 진짜 측정
2. **2단계** — 안정 확인 후 5~10곳 추가 (다양한 규모 섞어서)
3. **3단계** — 안정 확인 후 코드에서 `groupTag === 'beta_split_load'` 분기 제거 → 전체 적용

**각 단계 사이 권장 관찰 기간**: 3~7일.

**확대 SQL:**
```sql
-- 큰 회사 후보 찾기 (직원 수 많은 순)
SELECT c.id, c.email, c.company_name, c.group_tag
FROM companies c
WHERE c.status = 'active' AND c.group_tag IS NULL
ORDER BY c.id;

-- 베타 추가
UPDATE companies SET group_tag = 'beta_split_load' WHERE email = '...@...';

-- 베타 해제 (롤백)
UPDATE companies SET group_tag = NULL WHERE email = '...@...';

-- 현재 베타 그룹 조회
SELECT id, email, company_name FROM companies WHERE group_tag = 'beta_split_load';
```

### 관찰 포인트
- F12 콘솔에 `🛡️ 빈값 저장 차단`·`showAuthModal is not defined` 메시지 **재발 여부**
- 관리자 패널 [📊 모니터링] 새 source: `sbLoadRemainder` (재시도 경고), `sbSaveAll-rejected`, `sbSaveAll`(진입 가드 우회) — 발생 시 즉시 추적
- 베타 사용자가 보고하는 화면 이상 (특히 출퇴근·급여·안전교육·폴더)

### 핵심 파일·라인 (수정 시 참고)
- `js/app.js`
  - line 9 `CLIENT_BUILD` — 배포 시 갱신
  - line 30~37 `ESSENTIAL_KEYS`·`REMAINDER_KEYS`·`_remainderLoaded` 상수/플래그
  - `sbLoadEssential`·`sbLoadRemainder`·`sbLoadAll` (호환성 래퍼)
  - `_needsRemainder`·`_isBetaSplitLoad`·`_showRemainderPlaceholder`·`_REMAINDER_MESSAGES`
  - `nopro:remainder-loaded` 이벤트 핸들러 (gp() 위)
  - `sbSaveAll`·`safeItemSave` 진입 가드 (`_syncedSnapshot===null` return)
- `index.html` line 2302 `?v=...` — 배포 시 갱신
- `netlify/functions/data-load.js` line 67 `SERVER_BUILD` fallback — 배포 시 갱신

### 전체 확장 시 정리할 dead code
- group_tag 분기 (`doAuthLogin`·`initAuth`·`gp()` 가드)
- `_isBetaSplitLoad()` 헬퍼 자체
- `_REMAINDER_MESSAGES`·`_showRemainderPlaceholder` (계속 사용)
